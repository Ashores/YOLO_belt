...
class DFL(nn.Module):
    ...
    def forward(self, x,export):
        """Applies a transformer layer on input tensor 'x' and returns a tensor."""
        b, c, a = x.shape  # batch, channels, anchors
        if export == True:
            proj = torch.arange(self.c1, dtype=torch.float)
            return x.view(b, 4, self.c1, a).permute(0, 3,1,2).softmax(3).matmul(proj.type(x.dtype))
        else:
...
#         self.conv = nn.Conv2d(c1, 1, 1, bias=False).requires_grad_(False)
#         x = torch.arange(c1, dtype=torch.float)
#         self.proj = torch.arange(c1, dtype=torch.float)
#         self.conv.weight.data[:] = nn.Parameter(x.view(1, c1, 1, 1))
#         self.c1 = c1
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        # self.cv2 = nn.Conv2d(c1, c2, 1, 1, 0,bias=False)
...
class C2f_4(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        # self.cv3 = Conv(c2, int(c2/4), 1)
        # self.cv4 = Conv(int(c2/4), c2, 1)
        self.cv4 = Conv((2 + n) * self.c, c2, 1)    
        self.cv2 = Conv(c2, int(c2/4), 1)    
        self.cv3 = Conv(int(c2/4), c2, 1)

    def forward(self, x):
        """Forward pass of a YOLOv5 CSPDarknet backbone layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        z = self.cv2(self.cv4(torch.cat(y, 1)))
        z = self.cv3(z)
        # z = self.cv4(self.cv3(z))
        return z
...
class C2f_2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv4 = Conv((2 + n) * self.c, c2, 1)    
        self.cv2 = Conv(c2, int(c2/2), 1)    
        self.cv3 = Conv(int(c2/2), c2, 1)
...
class Detect(nn.Module):
    ...
    set_Distillation = False  # export mode
    shape = None
    ...
    def forward(self, x):
        ...
        if self.training or self.set_Distillation:
            return x
        ...
        # pdb.set_trace()
        if self.export == False:
            dbox = dist2bbox(self.dfl(box,self.export), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
            y = torch.cat((dbox, cls.sigmoid()), 1)
        else:
            dbox = dist2bbox(self.dfl(box,self.export), self.anchors.unsqueeze(0), xywh=True, dim=2) * self.strides
            y = torch.cat((dbox, cls.sigmoid().transpose(2,1)), 2)
